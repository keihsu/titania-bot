...
client.on('messageReactionAdd', async (reaction, user) => {
  ...
  if(reaction.message.channel.id === '847944722387435540' && reaction.message.id !== '847956278734225449') {
  ...
  if (roleId) {
    const guestRole = reaction.message.guild.roles.cache.find(role => role.id === '846898905521717249');
    var newRole = reaction.message.guild.roles.cache.find(role => role.id === roleId);
...
client.on('messageReactionRemove', async (reaction, user) => {
 ...
 var roleId = getRoleId(reaction);
  if (roleId) {
    ...
    if (!roles.has('846900178438193182')
...
client.on('messageCreate', async (message) => {
  if (message.channel.id === '847944722387435540'){
    ...
    if((firstWord === 'gather:' || firstWord === 'craft:') && content.length > 0) {
      message.react('🚧');
      message.react('✅');
    } else {
      message.delete();
